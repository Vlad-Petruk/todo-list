...
let defaultTodo = TodoFactory(
  "Open me...",
  "i know how you feel... but keep going, just one step at a time",
  "11/12/12",
  "top"
);
...
const domLoader = () => {
  ...
  const sectionTitle = createAndAppendElement(
    "div",
    "section-title",
    null,
    null,
  ...
  function renderNavigation(sections) {
    ...
    const createProjectBtn = createAndAppendElement(
      "button",
      "create-project-btn",
      null,
      "Create project",
    ...
    sections.forEach((section) => {
      ...
      // }
      const itemTitle = createAndAppendElement(
        "div",
        "item-title",
        null,
        section.title,
      ...
      if (section.id !== defaultSectionId) {
        const deleteTodoBtn = createAndAppendElement(
          "div",
          "delete-section-btn",
          null,
          "&times;",
    ...
    if (section.todos.length === 0) {
      ...
      section.todos.forEach((todo, index) => {
        ...
        todoElement.addEventListener("click", () =>
          openTodoDetailsModal(todo, index)
        );

        todoContainer.appendChild(checkbox);
        todoContainer.appendChild(todoElement);
        const buttonsElement = createAndAppendElement(
          "div",
          "buttons-el",
          null,
          null,
        ...
        if (todo.title !== "Open me...") {
          const dueDateinfo = createAndAppendElement(
            "div",
            "due-date-info",
            null,
            todo.dueDate,
  ...
  function openSectionCreateModal() {
    ...
    const modalTitle = createAndAppendElement(
      "h2",
      "modal-title",
      null,
      "Section form",
    ...
    const sectionForm = createAndAppendElement(
      "form",
      "todo-form",
      null,
      null,
    ...
    const todoSubmitBtn = createAndAppendElement(
      "button",
      "section-submit",
      null,
      "Submit",
    ...
    todoSubmitBtn.addEventListener("click", () => {
      ...
      if (sectionTitle.value === "") {
        ...
        showContent(
          sectionsFromStorage.value.find(
            (section) => section.id === sectionTitle.value
          )
        );
      }
    });
  ...
  function openTodoCreateModal() {
    ...
    const todoTitle = createAndAppendElement(
      "input",
      "title",
      "title",
      null,
    ...
    const todoDescription = createAndAppendElement(
      "textarea",
      "description",
      "description",
      null,
    ...
    const todoDueDate = createAndAppendElement(
      "input",
      "dueDate",
      "dueDate",
      null,
    ...
    const todoPriority = createAndAppendElement(
      "select",
      "priority",
      "priority",
      null,
    ...
    todoSubmitBtn.addEventListener("click", () => {
      ...
      showContent(
        sectionsFromStorage.value.find(
          (section) => section.id === sectionTitle.innerHTML
        )
      );
    });
  ...
  function deleteSection(sectionId) {
    ...
    const sectionIndex = sectionsFromStorage.value.findIndex(
    ...
    if (sectionIndex !== -1) {
      ...
      if (
        ...
        sectionsFromStorage.value.length > 1
      ) {
        sectionsFromStorage.value.splice(sectionIndex, 1); // Remove the section

        addToLocalStorage("sections", sectionsFromStorage.value);
        console.log(
          sections.find((section) => section.id === defaultSectionId)
        );
        onload();
        content.innerHTML = "";
        domLoader();
  ...
  function deleteTodo(index) {
    let sectionsFromStorage = getFromLocalStorage("sections");
    const section = sectionsFromStorage.value.find(
      (sec) => sec.id === sectionTitle.innerHTML
    );
    ...
    if (section) {
      ...
      if (!isAllTodos || todoToDelete.title !== "Open me...") {
        const allTodosSection = sectionsFromStorage.value.find(
          (sec) => sec.id === defaultSectionId
        );
        if (allTodosSection) {
          const allTodosIndex = allTodosSection.todos.findIndex(
  ...
  function populateSection(sectionId, todo) {
    ...
    if (sectionTitle.innerHTML === defaultSectionId) {
      let sectionsFromStorage = getFromLocalStorage("sections");
      showContent(
        sectionsFromStorage.value.find((section) => section.id === sectionId)
      );
    }
  }
